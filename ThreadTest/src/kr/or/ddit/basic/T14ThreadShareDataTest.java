...
public class T14ThreadShareDataTest {
	 ...
	 * 4. 가각의 스레드는 이 인스턴스의 참조값을 저장한 변수를 이용하여 공통 데이터를 사요한다.
	 * 
	 * ex) 원주율을 계산하는 스레드가 있고, 계산된 원주율을 출력하는 스레드가 있다. 
	 *     원주율을 계산한 후 이 값을 출력하는 프로그램을  작성하시오. (이때 원주율을 저장하는 객체가 필요하다)
	 */
...
class ShareData {
	private double result; // 원주율이 저장된 변수
	// volatile => 선언된 변수를 컴파일러의 최적화 대상에서 제외시킨다. 즉, 값이 변경되는 즉시 변수에 적용시킨다.(일종의 동기화)
	volatile private boolean isOk = false;	// 원주율 계산이 완료되었는지를 타나냄
