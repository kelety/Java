...
class SyncAccount {
	...
	// 동기화 영역에서 호출하는 메서드도 동기화 처리를 해 주어야 한다.   -->    getBalance()가 실행이 될때 동기화가 풀려버리는것 때메  public int getBalance()함수를 동기화 시켜야한다
	synchronized public boolean withdraw(int money) {
		if (balance >= money) { // 잔액이 충분할 경우...
			...
			balance -= money;
			System.out.println("메서드 안에서 balance = " + getBalance());      //getBalance()가 실행이 될때 동기화가 풀려버리는것 때메  public int getBalance()함수를 동기화 시켜야한다
