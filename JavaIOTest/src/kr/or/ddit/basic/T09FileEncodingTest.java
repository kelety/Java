...
public class T09FileEncodingTest {
	 ...
	 * 원래 한글 윈도우는 CP949 방식을 사용했는데, 윈도우를 개발한 마이크로소프트에서 EUC-KR 방식에서 확장하였기 때문에 MS949라고도 부른다.
	 * 한글 Window의 메모장에서 말하는 ANSI 인코딩이라 CP949(Code Page 949)를 말한다.
	 * - MS949 -> 윈도우의 기본 한글 인코딩방식(ANSI 계열)
	 * - UTF-8 -> 유니코드 UTF-8 인코딩 방식(영문자 및 숫자 : 1byte | 한글 : 3byte) => 가변적  
	 * - US-ASCII -> 영문전용 인코딩 방식
	 * 
	 * ANSI는 영어를 표기하기 위해 만든 코드로 규격 자체에 한글이 없었다가 나중에 여기에 EUC-KR, CP949 라는 식으로 한글이 포함된 방식이 생겨났다.
	*/	
